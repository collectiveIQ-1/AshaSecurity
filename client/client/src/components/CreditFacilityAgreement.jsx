...
function InlineInput({ value, onChange, onBlur, placeholder, className = "", disabled }) {
  return (
    <input
      value={value || ""}
...
export default function CreditFacilityAgreement({
  form,
  update,
  busy,
  onPrev,
  ...
  // Optional notifier from Wizard (defaults to window.alert)
  notify: notifyProp,
}) {
  ...
  // --- helpers ---
  const notify = (msg) => {
    if (typeof notifyProp === "function") return notifyProp(msg);
    window.alert(msg);
  };

  const digitsOnly = (v) => (v || "").replace(/[^\d]/g, "");

  const normalizeNumberInput = (raw, maxLen = 2) => {
  ...
  const validateDay = (v) => isInRange(v, 1, 31);
  const validateMonth = (v) => isInRange(v, 1, 12);

  const validateAllBeforeNext = () => {
    const d1 = (date?.day || "").trim();
    const m1 = (date?.month || "").trim();
    const d2 = (exec?.date || "").trim();
    const m2 = (exec?.month || "").trim();

    if (!validateDay(d1)) {
      return { ok: false, msg: "Date must be between 1 and 31." };
    }
    if (!validateMonth(m1)) {
      return { ok: false, msg: "Month must be between 1 and 12." };
    }
    if (!validateDay(d2)) {
      return { ok: false, msg: "Execution date must be between 1 and 31." };
    }
    if (!validateMonth(m2)) {
      return { ok: false, msg: "Execution month must be between 1 and 12." };
    }
    return { ok: true };
  };

  const handleNext = () => {
    const res = validateAllBeforeNext();
    if (!res.ok) {
      notify(res.msg);
  ...
  const onBlurValidateDay = (pathKey, value) => {
    const v = (value || "").trim();
    if (v === "") return;
    if (!validateDay(v)) {
      notify("Date must be between 1 and 31.");
      update(pathKey, "");
    }
  };

  const onBlurValidateMonth = (pathKey, value) => {
    const v = (value || "").trim();
    if (v === "") return;
    if (!validateMonth(v)) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-3xl border border-zinc-800 bg-zinc-950/60 p-8">
        <div className="text-center text-base font-semibold tracking-wide text-zinc-100">
          AGREEMENT - CREDIT FACILITY
        </div>

        {/* This AGREEMENT line */}
        <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-3 text-sm text-zinc-300">
          <span>This AGREEMENT is made and entered</span>
          <span>into on this</span>

          <InlineInput
            value={date.day}
            onChange={(v) => update("creditFacility.date.day", normalizeNumberInput(v, 2))}
            onBlur={() => onBlurValidateDay("creditFacility.date.day", date.day)}
            placeholder="Date"
            disabled={busy}
            className="w-28"
          />
          ...
          <InlineInput
            value={date.month}
            onChange={(v) => update("creditFacility.date.month", normalizeNumberInput(v, 2))}
            onBlur={() => onBlurValidateMonth("creditFacility.date.month", date.month)}
            placeholder="Month"
            disabled={busy}
            className="w-40"
          />
        ...
        {/* By and Between */}
        <div className="mt-8">
          <div className="text-sm font-semibold text-zinc-100">By and Between</div>

          <div className="mt-4 text-xs leading-relaxed text-zinc-300/90">
          ...
          <div className="mt-5 text-sm font-semibold text-zinc-100">and</div>

          {/* Client line with inputs */}
          <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-3 text-xs text-zinc-300/90">
            <InlineInput
              value={client.name}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-zinc-300/90">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-zinc-300/90">
            {/* <span>
          ...
          <div className="mt-5 text-xs text-zinc-300/90">
        ...
        <div className="mt-5 text-xs text-zinc-300/90">
          ...
          <p>5. Any outstanding credit payments above T+7 will be charged at an interest rate of 24% annually.</p>
        </div>
        ...
        <div className="mt-8 text-xs text-zinc-300/90">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-zinc-300/90">
          <span>has affixed its common seal / placed his / her signature hereunto and to one other on this</span>

          <InlineInput
            value={exec.date}
            onChange={(v) => update("creditFacility.execution.date", normalizeNumberInput(v, 2))}
            onBlur={() => onBlurValidateDay("creditFacility.execution.date", exec.date)}
            placeholder="Date"
            disabled={busy}
            className="w-24"
          />
          ...
          <InlineInput
            value={exec.month}
            onChange={(v) => update("creditFacility.execution.month", normalizeNumberInput(v, 2))}
            onBlur={() => onBlurValidateMonth("creditFacility.execution.month", exec.month)}
            placeholder="Month"
            disabled={busy}
            className="w-32"
          />
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="text-xs text-zinc-300 mb-2">Authorised Signatory of the Client</div>
            <FileUpload
              label=""
              accept="image/*,.pdf"
              file={cfPrincipalSig}
          ...
          <div>
            <div className="text-xs text-zinc-300 mb-2">Authorised Signatory of the Company</div>
            <FileUpload
              label=""
              accept="image/*,.pdf"
              file={cfFirmSig}
        ...
        <br></br>
        <div className="text-xs text-zinc-300 mb-2">Witness 1</div>
        ...
        <br></br>
        <div className="text-xs text-zinc-300 mb-2">Witness 2</div>
